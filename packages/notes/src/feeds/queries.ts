...
export async function getNewFeed(
  ...
  const { limit: validatedLimit, offsetTime } = FeedScaffolding.validateParams(limit, cursor)
  ...
  try {
    const results = await ctx.notesDb.db
      .selectFrom('record')
      .select([
        sql`json_extract(record, '$.uri')`.as('targetUri'),
        'indexedAt'
      ])
    ...
    const feedResult = FeedScaffolding.processFeedResults(results, validatedLimit, 'indexedAt')
...
export async function getNeedsYourHelpFeed(
  ...
  try {
    ...
    if (ctx.userDid) {
      ...
      const ratedProposalUris = new Set(userVotes.map(v => v.proposalUri as string))
      ...
      for (const score of postsWithScores) {
        if (!ratedProposalUris.has(score.proposalUri)) {
          ...
          postsWithUnratedNotes.set(score.targetUri, Math.max(currentTime, score.scoreEventTime))
        }
      }
      ...
      const feedResult = FeedScaffolding.processFeedResults(results, validatedLimit, 'scoreEventTime')
      ...
      const results = await ctx.scoresDb.db
        .selectFrom('score as s')
        .select(['s.targetUri', sql<number>`MAX(s.scoreEventTime)`.as('scoreEventTime')])
        .where('s.status', '=', 'needs_more_ratings')
        .where('s.scoreEventTime', '<', offsetTime)
        .groupBy('s.targetUri')
